...
function _parsepngstream($f, $file)
{
	...
	if($ct>=4)
	{
		...
		unset($data);
		$data = gzcompress($color, 0);
		$info['smask'] = gzcompress($alpha, 0);
...
function _putpages()
{
	...
	for($n=1;$n<=$nb;$n++)
	{
		...
		$this->_out('endobj');
		// Page content
		$p = ($this->compress) ? gzcompress($this->pages[$n], 0) : $this->pages[$n];
		$this->_newobj();
...
function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		// TrueType embedded SUBSETS or FULL
			...
			$fontstream = gzcompress($ttfontstream, 0);
			...
			$cidtogidmap = gzcompress($cidtogidmap, 0);
			$this->_newobj();
...
function _putimage(&$info)
{
	...
	{
		$filter = ($this->compress) ? '/Filter /FlateDecode ' : '';
		$pal = ($this->compress) ? gzcompress($info['pal'], 0) : $info['pal'];
		$this->_newobj();
